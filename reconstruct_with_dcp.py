...
#     """
#     在 cam->world 位姿 T 上左乘一个小噪声 ΔT：
#     T_noisy = ΔT * T
#     rot_sigma_deg: 旋转噪声标准差（度）
#     trans_sigma:   平移噪声标准差（米）
#     """
#     # ---- 1) 采一个小角度轴角 ----
#     theta = np.deg2rad(rot_sigma_deg) * np.random.randn()  # 标准差 ≈ rot_sigma_deg
...
#     """用前 max_frames 帧的 depth + T_wB 估计整场景 bbox"""
...
#         # cam -> world（和你导出 world_frame 的式子一致）
...
#     out_dir = "debug_single_tsdf"
#     os.makedirs(out_dir, exist_ok=True)

#     # ====== Step1-2: 用前 M 帧估计整个场景 bbox ======
#     N = max(1, len(dataset))
#     M = N  # 比如先用前 200 帧
...
#     # log.info(f"[TSDF-1frame] vol_min={vol_min}")
#     # log.info(f"[TSDF-1frame] vol_max={vol_max}")
...
#     )
#     # ====== 开关：是否给 pose 加噪声 ======
#     ROT_SIGMA_DEG = 1.0  # 每帧 ≈ 1° 标准差
#     TRANS_SIGMA_M = 0.01  # 每帧 ≈ 1 cm 标准差

#     T_icp_list = build_icp_poses(dataset, N)
#     # N = 30
#     print(f"[TSDF] Integrating {N} frames... (USE_NOISE={(POSE_MODE == "icp_noise")})")
...
#         # 2) 取这一帧自己的位姿（真值 cam->world）
#         # data_i = dataset[idx]
#         # *_, T_wA_i, T_wB_i = data_i
#         # cam_pose = T_wB_i
#         # cam_pose = T_icp_list[idx]  # ← 核心：用 ICP 的 pose，而不是 T_wB_i
...
#                 T_icp_list[idx], rot_sigma_deg=ROT_SIGMA_DEG, trans_sigma=TRANS_SIGMA_M
#             )
#             # 简单打印一下每 50 帧的噪声量级
#             if idx % 50 == 0:
...
#     log.info(
#         f"[Step1] Saved single-frame TSDF mesh → {tsdf_path}, "
...
#     Build per-frame cam->world poses by chaining pairwise point-to-point ICP.
#     The first frame uses GT T_wB_0 as world origin; later frames are T_ji @ T_{i}.
#     """
#     _, _, _, _, _, _, _, _, _, _, T_wA_0, T_wB_0 = dataset[0]
#     T_icp = [T_wB_0.copy()]
#
#     for i in range(N - 1):
#         # Depth -> cam point clouds
...
#
#         # ICP: estimate relative transform (cam_i -> cam_j)
#         R_ji, t_ji = icp_point_to_point(pc_i_ds, pc_j_ds)
#         T_ji = np.eye(4, dtype=np.float32)
#         T_ji[:3, :3] = R_ji
#         T_ji[:3, 3] = t_ji
#
#         # Accumulate to world frame: cam_j = T_ji * cam_i
#         T_j = T_ji @ T_icp[-1]
#         T_icp.append(T_j)
#
#     return T_icp
